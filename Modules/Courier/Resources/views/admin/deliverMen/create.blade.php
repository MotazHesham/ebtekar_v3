...
<div class="card">
    <div class="card-header">
        {{ __('global.create') }} {{ __('cruds.deliverMan.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route('admin.deliver-men.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select name="shipping_partner_id" class="form-control">
                    ...
                    @foreach ($shippingPartners as $id => $name)
                        <option value="{{ $id }}" @selected(old('shipping_partner_id') == $id)>{{ $name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="status" class="form-control">
                    <option value="active" @selected(old('status', 'active') === 'active')>{{ __('cruds.shippingPartner.fields.active') }}</option>
                    <option value="inactive" @selected(old('status') === 'inactive')>{{ __('cruds.shippingPartner.fields.inactive') }}</option>
                </select>
            </div>
            <div class="form-group">
