                        ...
                        <tr data-entry-id="{{ $homeCategory->id }}">
                            ...
                            <td>
                                @can('home_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.home-categories.show', $homeCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('home_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.home-categories.edit', $homeCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('home_category_delete')
                                    <form action="{{ route('admin.home-categories.destroy', $homeCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-HomeCategory:not(.ajaxTable)').DataTable({ buttons: dtButtons })
