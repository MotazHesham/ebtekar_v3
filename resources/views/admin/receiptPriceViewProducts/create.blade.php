...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.receiptPriceViewProduct.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.receipt-price-view-products.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('receipt_price_view') ? 'is-invalid' : '' }}" name="receipt_price_view_id" id="receipt_price_view_id" required>
                    @foreach($receipt_price_views as $id => $entry)
                        <option value="{{ $id }}" {{ old('receipt_price_view_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
