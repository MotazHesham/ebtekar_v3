...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.rClient.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.r-clients.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('manage_type') ? 'is-invalid' : '' }}" name="manage_type" id="manage_type">
                    ...
                    @foreach(App\Models\RClient::MANAGE_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('manage_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
