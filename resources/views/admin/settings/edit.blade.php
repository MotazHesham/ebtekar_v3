    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.settings.update", [$setting->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('key') ? 'is-invalid' : '' }}" type="text" name="key" id="key" value="{{ old('key', $setting->key) }}">
                @if($errors->has('key'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('options') ? 'is-invalid' : '' }}" type="text" name="options" id="options" value="{{ old('options', $setting->options) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lang') ? 'is-invalid' : '' }}" type="text" name="lang" id="lang" value="{{ old('lang', $setting->lang) }}">
                @if($errors->has('lang'))
                    <div class="invalid-feedback">
                        {{ $errors->first('lang') }}
                    </div>
            ...
            <div class="form-group">
                <label>{{ trans('cruds.setting.fields.select') }}</label>
                <select class="form-control {{ $errors->has('select') ? 'is-invalid' : '' }}" name="select" id="select">
                    ...
                    @foreach(App\Models\Setting::SELECT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('select', $setting->select) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.setting.fields.select_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="group_name">{{ trans('cruds.setting.fields.group_name') }}</label>
                <input class="form-control {{ $errors->has('group_name') ? 'is-invalid' : '' }}" type="text" name="group_name" id="group_name" value="{{ old('group_name', $setting->group_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('order_level') ? 'is-invalid' : '' }}" type="number" name="order_level" id="order_level" value="{{ old('order_level', $setting->order_level) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('grid_col') ? 'is-invalid' : '' }}" type="text" name="grid_col" id="grid_col" value="{{ old('grid_col', $setting->grid_col) }}">
