    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.excel-files.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(App\Models\ExcelFile::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', 'social_delivery') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('results') ? 'is-invalid' : '' }}" name="results" id="results" required>{{ old('results') }}</textarea>
