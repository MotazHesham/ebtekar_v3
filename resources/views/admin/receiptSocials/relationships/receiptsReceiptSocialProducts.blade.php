...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-receiptsReceiptSocialProducts">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($receiptSocialProducts as $key => $receiptSocialProduct)
                        <tr data-entry-id="{{ $receiptSocialProduct->id }}">
                            <td>
                            ...
                            <td>
                                @can('receipt_social_product_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.receipt-social-products.show', $receiptSocialProduct->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('receipt_social_product_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.receipt-social-products.edit', $receiptSocialProduct->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('receipt_social_product_delete')
                                    <form action="{{ route('admin.receipt-social-products.destroy', $receiptSocialProduct->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-receiptsReceiptSocialProducts:not(.ajaxTable)').DataTable({ buttons: dtButtons })
