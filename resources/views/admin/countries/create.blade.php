    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.countries.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('code_cost') ? 'is-invalid' : '' }}" type="number" name="code_cost" id="code_cost" value="{{ old('code_cost', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\Country::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', 'countries') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('website') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="website" id="website" value="1" required {{ old('website', 0) == 1 || old('website') === null ? 'checked' : '' }}>
