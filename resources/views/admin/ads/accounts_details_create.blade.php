...
<div class="card">
    <div class="card-header">
        {{ trans('Create Account Detail') }} — {{ $selectedAccount->name }}
        <a class="btn btn-default btn-sm float-right" href="{{ route('admin.ads.accounts.index', ['account_id' => $selectedAccount->id]) }}">
            <i class="fa fa-arrow-left"></i> {{ trans('Back') }}
        </a>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('admin.ads.accounts.details.store', $selectedAccount->id) }}">
            @csrf
            <div class="row">
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                            ...
                            <option value="campaign" {{ old('type') == 'campaign' ? 'selected' : '' }}>{{ trans('Campaign') }}</option>
                            <option value="ad_set" {{ old('type') == 'ad_set' ? 'selected' : '' }}>{{ trans('Ad Set') }}</option>
                            <option value="ad" {{ old('type') == 'ad' ? 'selected' : '' }}>{{ trans('Ad') }}</option>
                        </select>
                        @if($errors->has('type'))
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control {{ $errors->has('parent_id') ? 'is-invalid' : '' }}" name="parent_id" id="parent_id">
                            ...
                            @foreach(($parentOptions ?? collect()) as $opt)
                                ...
                                @endphp
                                <option value="{{ $opt->id }}" {{ (string)old('parent_id') === (string)$opt->id ? 'selected' : '' }}>{{ $opt->name }} ({{ $typeLabel }}){{ $parentPart }}</option>
                            @endforeach
                        </select>
