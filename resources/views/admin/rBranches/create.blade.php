...
<div class="card">
    <div class="card-header">
        {{ __('global.create') }} {{ __('cruds.rBranch.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.r-branches.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required">{{ __('cruds.rBranch.fields.type') }}</label>
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    <option value disabled {{ old('type', null) === null ? 'selected' : '' }}>{{ __('global.pleaseSelect') }}</option>
                    @foreach(App\Models\RBranch::TYPE_SELECT as $key => $label)
                ...
                @endif
                <span class="help-block">{{ __('cruds.rBranch.fields.type_helper') }}</span>
            </div>
            <div class="form-group">
