...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.orders.update", [$order->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="shipping_country_name">{{ trans('cruds.order.fields.shipping_country_name') }}</label>
                <input class="form-control {{ $errors->has('shipping_country_name') ? 'is-invalid' : '' }}" type="text" name="shipping_country_name" id="shipping_country_name" value="{{ old('shipping_country_name', $order->shipping_country_name) }}" required>
                @if($errors->has('shipping_country_name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('shipping_country_name') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.shipping_country_name_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="shipping_country_cost">{{ trans('cruds.order.fields.shipping_country_cost') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.shipping_country_cost_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="printing_times">{{ trans('cruds.order.fields.printing_times') }}</label>
                <input class="form-control {{ $errors->has('printing_times') ? 'is-invalid' : '' }}" type="number" name="printing_times" id="printing_times" value="{{ old('printing_times', $order->printing_times) }}" step="1">
                @if($errors->has('printing_times'))
                    <div class="invalid-feedback">
                        {{ $errors->first('printing_times') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.printing_times_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="date_of_receiving_order">{{ trans('cruds.order.fields.date_of_receiving_order') }}</label>
                <input class="form-control date {{ $errors->has('date_of_receiving_order') ? 'is-invalid' : '' }}" type="text" name="date_of_receiving_order" id="date_of_receiving_order" value="{{ old('date_of_receiving_order', $order->date_of_receiving_order) }}">
                @if($errors->has('date_of_receiving_order'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date_of_receiving_order') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.date_of_receiving_order_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="excepected_deliverd_date">{{ trans('cruds.order.fields.excepected_deliverd_date') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.excepected_deliverd_date_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label>{{ trans('cruds.order.fields.deposit_type') }}</label>
                <select class="form-control {{ $errors->has('deposit_type') ? 'is-invalid' : '' }}" name="deposit_type" id="deposit_type">
                    <option value disabled {{ old('deposit_type', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.deposit_type_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="deposit_amount">{{ trans('cruds.order.fields.deposit_amount') }}</label>
            ...
            <div class="form-group">
                <label for="total_cost_by_seller">{{ trans('cruds.order.fields.total_cost_by_seller') }}</label>
                <input class="form-control {{ $errors->has('total_cost_by_seller') ? 'is-invalid' : '' }}" type="number" name="total_cost_by_seller" id="total_cost_by_seller" value="{{ old('total_cost_by_seller', $order->total_cost_by_seller) }}" step="0.01">
                @if($errors->has('total_cost_by_seller'))
                    <div class="invalid-feedback">
                        {{ $errors->first('total_cost_by_seller') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.total_cost_by_seller_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="commission">{{ trans('cruds.order.fields.commission') }}</label>
                <input class="form-control {{ $errors->has('commission') ? 'is-invalid' : '' }}" type="number" name="commission" id="commission" value="{{ old('commission', $order->commission) }}" step="0.01">
                @if($errors->has('commission'))
                    <div class="invalid-feedback">
                        {{ $errors->first('commission') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.commission_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="extra_commission">{{ trans('cruds.order.fields.extra_commission') }}</label>
                <input class="form-control {{ $errors->has('extra_commission') ? 'is-invalid' : '' }}" type="number" name="extra_commission" id="extra_commission" value="{{ old('extra_commission', $order->extra_commission) }}" step="0.01">
                @if($errors->has('extra_commission'))
                    <div class="invalid-feedback">
                        {{ $errors->first('extra_commission') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.extra_commission_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $order->note) }}</textarea>
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
            ...
            <div class="form-group">
                <label for="delay_reason">{{ trans('cruds.order.fields.delay_reason') }}</label>
                <textarea class="form-control {{ $errors->has('delay_reason') ? 'is-invalid' : '' }}" name="delay_reason" id="delay_reason">{{ old('delay_reason', $order->delay_reason) }}</textarea>
                @if($errors->has('delay_reason'))
                    <div class="invalid-feedback">
                        {{ $errors->first('delay_reason') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.delay_reason_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label class="required" for="shipping_country_id">{{ trans('cruds.order.fields.shipping_country') }}</label>
                <select class="form-control select2 {{ $errors->has('shipping_country') ? 'is-invalid' : '' }}" name="shipping_country_id" id="shipping_country_id" required>
                    @foreach($shipping_countries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('shipping_country_id') ? old('shipping_country_id') : $order->shipping_country->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.shipping_country_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="preparer_id">{{ trans('cruds.order.fields.preparer') }}</label>
                <select class="form-control select2 {{ $errors->has('preparer') ? 'is-invalid' : '' }}" name="preparer_id" id="preparer_id">
                    @foreach($preparers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('preparer_id') ? old('preparer_id') : $order->preparer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.preparer_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="delivery_man_id">{{ trans('cruds.order.fields.delivery_man') }}</label>
                <select class="form-control select2 {{ $errors->has('delivery_man') ? 'is-invalid' : '' }}" name="delivery_man_id" id="delivery_man_id">
                    @foreach($delivery_men as $id => $entry)
                        <option value="{{ $id }}" {{ (old('delivery_man_id') ? old('delivery_man_id') : $order->delivery_man->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.delivery_man_helper') }}</span>
            </div>
            <div class="form-group">
