...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.receiptOutgoingProduct.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.receipt-outgoing-products.update", [$receiptOutgoingProduct->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="description">{{ trans('cruds.receiptOutgoingProduct.fields.description') }}</label>
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $receiptOutgoingProduct->description) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $receiptOutgoingProduct->price) }}" step="0.01" required>
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $receiptOutgoingProduct->quantity) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_cost') ? 'is-invalid' : '' }}" type="number" name="total_cost" id="total_cost" value="{{ old('total_cost', $receiptOutgoingProduct->total_cost) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('receipt_outgoing') ? 'is-invalid' : '' }}" name="receipt_outgoing_id" id="receipt_outgoing_id">
                    @foreach($receipt_outgoings as $id => $entry)
                        <option value="{{ $id }}" {{ (old('receipt_outgoing_id') ? old('receipt_outgoing_id') : $receiptOutgoingProduct->receipt_outgoing->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
